...
train_cot = train_log.groupby(['USRID']).size().reset_index(name='counts')
test_cot = test_log.groupby(['USRID']).size().reset_index(name='counts')
# log['counts']=log['counts']/log['counts'].max()
...
train = pd.merge(train,train_cot,on=['USRID'],how='left',copy=False)
train = train.fillna(0)
test = pd.merge(test,test_cot,on=['USRID'],how='left',copy=False)
test = test.fillna(0)
